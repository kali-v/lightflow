        ...
        for (int n = 0; n < filter.shape_[0]; n++) {
            for (int ch = 0; ch < x_channels; ch++) {
                for (int i = 0; i < nrows; i++) {
                    float* rtmp_data = &res.data_.data()[xn * filter.shape_[0] * out_size + n * out_size + i * nrows];
                    ...
                    for (int x_row = std::max(0, ps0); x_row < x_height && x_row < fil_height + ps0; x_row++) {
                        ...
                        for (int j = 0; j < ncols; j++) {
                            ...
                            for (x_col = std::max(0, ps1); x_col < x_width - 8 && x_col - ps1 < fil_width - 8;
                                 x_col += 8) {
                                resvec = _mm256_fmadd_ps(_mm256_loadu_ps(&x.data_[x_off + x_col]),
                                                         _mm256_loadu_ps(&filter.data_[f_off - ps1 + x_col]), resvec);
                            }
                            for (; x_col < x_width && x_col - ps1 < fil_width; x_col++) {
                                rtmp_data[j] += x.data_[x_off + x_col] * filter.data_[f_off - ps1 + x_col];
                            }
